...
while True:
    ...
    flag_moments, y_pts_moments, x_pts_moments, areas = ltools.direction_by_moments(
        mask_moments, remove_outliers=True)

    if flag_moments == 1:

        # Calculate yaw
        yaw_moments = ltools.yaw_angle_rads_alternative(
            x_pts_moments, y_pts_moments)

        # Calculate x, y and z
        x_near, y_near, x, y, z = ltools.get_xyz(
            mask_moments, intrinsics, A_known, x_pts_moments, y_pts_moments, areas)
...
<<<<<<< HEAD
        ...
        ltools.draw_circles(direction_by_moments_img,
                            x_pts_moments, y_pts_moments)
...
>>>>>>> 88600a321d39bb40da0ab847d83c24e28df6e752
        ...
        coord_txt = "x:" + str(round(x, 2)) + " y:" + \
            str(round(y, 2)) + " z:" + str(round(z, 2)) + "(m)"
        orient_txt = "yaw:" + str(round(yaw_moments, 2)) + "(rads)"
        cv.putText(direction_by_moments_img, coord_txt, (20, 30),
                   cv.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 50), 2)
        cv.putText(direction_by_moments_img, orient_txt, (20, 70),
                   cv.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 50), 2)
        cv.putText(direction_by_moments_img, fps_txt, (20, 110),
                   cv.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 50), 2)
